...
public final class FinderUtil
{
    ...
    /**
     * Queries a provided instance of {@link net.dv8tion.jda.api.JDA JDA} for
     * {@link net.dv8tion.jda.api.entities.channel.middleman.AudioChannel VoiceChannel}s.<p>
     *
     * If a {@link net.dv8tion.jda.api.sharding.ShardManager ShardManager} is available this will query across that
     * instead of the JDA instance.
     *
     ...
     */
    public static List<AudioChannel> findVoiceChannels(String query, JDA jda)
    {
     ...
     * @return A possibly-empty {@link java.util.List List} of VoiceChannels found by the query from the provided JDA instance.
     */
    public static List<AudioChannel> findShardVoiceChannels(String query, JDA jda)
    {
    ...
    /**
     * Queries a provided {@link net.dv8tion.jda.api.entities.Guild Guild} for
     * {@link net.dv8tion.jda.api.entities.channel.middleman.AudioChannel VoiceChannel}s.
     *
     * <p>The standard search does not follow any special cases.
     *
     * @param  query
     ...
     */
    public static List<AudioChannel> findVoiceChannels(String query, Guild guild)
    {
        if(DISCORD_ID.matcher(query).matches())
        {
            AudioChannel vc = guild.getChannelById(AudioChannel.class, query);
            if(vc!=null)
                return Collections.singletonList(vc);
        }

        List<AudioChannel> vcs = genericVoiceChannelSearch(query, guild.getVoiceChannelCache());

        if (!vcs.isEmpty()) {
            return vcs;
        }

        return genericVoiceChannelSearch(query, guild.getStageChannelCache());
    }

    private static List<AudioChannel> jdaVoiceChannelSearch(String query, JDA jda, boolean useShardManager)
    {
        ShardManager manager = useShardManager? jda.getShardManager() : null;

        if(DISCORD_ID.matcher(query).matches())
        {
            AudioChannel vc = manager != null? manager.getChannelById(AudioChannel.class, query) : jda.getChannelById(AudioChannel.class, query);
            if(vc!=null)
                return Collections.singletonList(vc);
        }

        List<AudioChannel> vcs = genericVoiceChannelSearch(query, manager != null ? manager.getVoiceChannelCache() : jda.getVoiceChannelCache());

        if (!vcs.isEmpty()) {
            return vcs;
        }

        return genericVoiceChannelSearch(query, manager != null? manager.getStageChannelCache() : jda.getStageChannelCache());
    }

    private static List<AudioChannel> genericVoiceChannelSearch(String query, SnowflakeCacheView<? extends AudioChannel> cache)
    {
        ArrayList<AudioChannel> exact = new ArrayList<>();
        ArrayList<AudioChannel> wrongcase = new ArrayList<>();
        ArrayList<AudioChannel> startswith = new ArrayList<>();
        ArrayList<AudioChannel> contains = new ArrayList<>();
        String lowerquery = query.toLowerCase();
        cache.forEach((vc) -> {
    ...
    /**
     * Queries a provided instance of {@link net.dv8tion.jda.api.JDA JDA} for
     * {@link net.dv8tion.jda.api.entities.emoji.RichCustomEmoji Emote}s.<p>
     *
     * If a {@link net.dv8tion.jda.api.sharding.ShardManager ShardManager} is available this will query across that
     * instead of the JDA instance.
     *
     ...
     *     Emote and that the {@link net.dv8tion.jda.api.entities.emoji.RichCustomEmoji#getName() name} of the Emote is equal to the
    ...
    /**
     * Queries a provided {@link net.dv8tion.jda.api.entities.Guild Guild} for
     * {@link net.dv8tion.jda.api.entities.emoji.RichCustomEmoji Emote}s.
     *
     * <p>The following special case is applied before the standard search is done:
     * <ul>
     *     <li>Emote Mention: Query provided matches a :emote: mention (more specifically {@literal <:emoteName:emoteID>}).
