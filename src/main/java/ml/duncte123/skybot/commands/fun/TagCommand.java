...
public class TagCommand extends Command {
    ...
    public TagCommand(Variables variables) {
        ...
        variables.getDatabaseAdapter().loadTags().thenAccept((tags) -> {
    ...
    private void removeTag(CommandContext ctx, String tagName) {
        ...
        ctx.getDatabaseAdapter().deleteTag(tag).thenAccept((pair) -> {
            if (!pair.getFirst()) {
    ...
    private void createTag(CommandContext ctx) {
        ...
        ctx.getDatabaseAdapter().createTag(newTag).thenAccept((pair) -> {
            if (!pair.getFirst()) {
